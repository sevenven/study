...
logger.info("Groovy版本: " + GroovySystem.version + ", 输入参数 - partyMembersBaseId: " + partyMembersBaseId + ", fromCellId: " + fromCellId + ", newMthUid: " + newMthUid);

// 更新党组织关系
def prevCur = callService("app_mgt503l5m8", "t_member_transfer_history_66g3i3yc_selectMore", JsonOutput.toJson([
  "page_index": 1,
  "page_size": 1,
  "query_criteria": [
    [
...
// 更新党组织关系转移表
prevCur[0] && callService("app_mgt503l5m8", "t_member_transfer_history_66g3i3yc_update", [uid: prevCur[0].uid, is_current: '0'])
...
// 处理领导班子的通用函数
def handleLeadership(partyMembersBaseId, prevCur, orgId, orgName, leader, adminIds, positionFields) {
  // 获取当前届信息
  ...
  if (curTerm && curTerm.size() > 0) {
    ...
    leadershipList.each { leadership ->
      if (leadership.name_id == partyMembersBaseId) {
        ...
        // 更新党员职务信息
        if (prevCur[0]) {
          def updateParams = [uid: prevCur[0].uid]
          // 动态设置需要清空的职务字段
...
handleLeadership(
  ...
  (fromCellInfo.parent_org_category_code == 'party_branch') ? [branch_party_position_code: null, branch_position_weight: 100] : [committee_party_position_code: null, committee_position_weight: 100]
)

// 处理党委班子&职务信息
if (fromCellInfo.parent_org_category_code == 'party_branch') {  
  handleLeadership(
		partyMembersBaseId,
		prevCur,
    fromCellInfo.grandparent_party_org_id,
    fromCellInfo.grandparent_party_org_name,
    fromCellInfo.grandparent_party_org_leader,
    fromCellInfo.grandparent_party_org_admin_id,
    [committee_party_position_code: null, committee_position_weight: 100]
  )
}
